...
const EditableContent = ({ content, setContent }) => {
  ...
  return (
    <div className="editable-content" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      {editing ? (
        <>
          <textarea className="edit-textarea" value={newContent} onChange={(e) => setNewContent(e.target.value)} />
          <button className="save-button" onClick={handleSave}>Save</button>
        </>
      ) : (
        <>
          <p className="editable-text">{content}</p>
          {hovered && <button className="edit-button" onClick={() => { setNewContent(content); setEditing(true); }}>Edit</button>}
        </>
      )}
    </div>
  );
...
const EditableTitle = ({ title, setTitle }) => {
  ...
  return (
    <div className="editable-title-wrapper" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      {editing ? (
        <>
        ...
        <>
          <h2 className="editable-title">{title}</h2>
          {hovered && <button className="edit-button" onClick={() => { setNewTitle(title); setEditing(true); }}>Edit</button>}
        </>
      )}
    </div>
  );
